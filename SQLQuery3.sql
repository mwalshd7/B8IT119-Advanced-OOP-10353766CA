...
CREATE TRIGGER trgAfterDelete ON [dbo].[Student]
AFTER DELETE
AS
declare @id int;
...
select @id=d.StudentId from deleted d;
select @fn=d.StudentFirstName from deleted d;
select @sn=d.StudentSurname from deleted d;
select @em=d.StudentEmail from deleted d;
select @ph=d.StudentPhone from deleted d;
select @ad1=d.StudentAddress1 from deleted d;
select @ad2=d.StudentAddress2 from deleted d;
select @cy=d.StudentCity from deleted d;
select @ct=d.StudentCounty from deleted d;
select @lv=i.StudentLevel from inserted i;
select @co=i.StudentCourse from inserted i;
set @aa='Record Deleted';
INSERT INTO Student_Audit
(StudentId,StudentFirstName,StudentSurname, StudentEmail, StudentPhone,StudentAddress1, StudentAddress2,StudentCity,StudentCounty,StudentLevel, StudentCourse, AuditAction, AuditDate )
VALUES
(@id,@fn,@sn,@em,@ph,@ad1,@ad2,@cy,@ct,@lv,@co,@aa,getdate());
...
CREATE TRIGGER trgAfterUpdate ON [dbo].[Student]
AFTER UPDATE
AS
declare @id int;
declare @fn NVARCHAR (50);
